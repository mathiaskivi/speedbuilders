...
public class SetupListener2 implements Listener {
	...
	public void onPlayerInteract(PlayerInteractEvent e) {
		...
		if (player.getItemInHand().getItemMeta().getDisplayName().equals("Set spawnpoint location for guardian plot")) {
			...
			if (action == Action.LEFT_CLICK_BLOCK) {
				String arenaName = plugin.getBungeeCord().setup.get(player.getName());

				double x = e.getClickedBlock().getLocation().getBlockX() + 0.5;
				double y = e.getClickedBlock().getLocation().getBlockY() + 10;
				double z = e.getClickedBlock().getLocation().getBlockZ() + 0.5;

				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.spawnpoint.world", e.getClickedBlock().getLocation().getWorld().getName());
				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.spawnpoint.x", x);
				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.spawnpoint.y", y);
				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.spawnpoint.z", z);
				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.spawnpoint.pitch", 0.0);
				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.spawnpoint.yaw", 0.0);
				new BukkitRunnable() {
					public void run() {
			...
			if (action == Action.LEFT_CLICK_BLOCK) {
				...
				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.template-area.x1", x1);
				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.template-area.y1", y1);
				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.template-area.z1", z1);
				new BukkitRunnable() {
					public void run() {
						plugin.getConfigManager().saveConfig("maps.yml");
					}
				...
				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.template-area.x2", x2);
				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.template-area.y2", y2);
				plugin.getConfigManager().getConfig("maps.yml").set("maps." + arenaName + ".plots.guardian.template-area.z2", z2);
				new BukkitRunnable() {
					public void run() {
						plugin.getConfigManager().saveConfig("maps.yml");
					}
			...
			if (action == Action.LEFT_CLICK_BLOCK) {
				plugin.getBungeeCord().location1 = e.getClickedBlock().getLocation();
			...
			if (action == Action.LEFT_CLICK_BLOCK) {
				plugin.getBungeeCord().location2 = e.getClickedBlock().getLocation();
				...
				if (plugin.getBungeeCord().location3 != null) {
				...
				plugin.getBungeeCord().location3 = e.getClickedBlock().getLocation();
				...
				if (plugin.getBungeeCord().location2 != null) {
					player.sendMessage(ChatColor.translateAlternateColorCodes('&', translate("PREFIX-CHAT") + "&7Type &f/sb setup plot-area <plot> &7to save both plot area positions into configuration file."));
				}
			}
